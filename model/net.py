...
from . import XNM_composite_modules as XNM_modules
from . import composite_modules as NKM_modules
from . import hierarchical_module as HCNMN_modules
from . import concept_modules as Concept_modules
...
class NKMNet(nn.Module):
    ...
    def forward(self, questions, questions_len, vision_feat, relation_mask, concepts):
        """
        Args:
            questions [Tensor] (batch_size, seq_len)
            questions_len [Tensor] (batch_size)
        ...
        aff_v = torch.zeros(batch_size, self.num_concepts)
        ...
        for t in range(self.T_ctrl):
            ...
            # run all modules
            res = [f(vision_feat.permute(0,2,1), feat_inputs, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem, aff_v, concepts) for f in self.module_funcs]
            att_stack_avg = torch.sum(module_prob.view(self.num_module,batch_size,1,1,1) * torch.stack([r[0] for r in res]), dim=0)
            stack_ptr_avg = torch.sum(module_prob.view(self.num_module,batch_size,1) * torch.stack([r[1] for r in res]), dim=0)
            stack_ptr_avg = modules._sharpen_ptr(stack_ptr_avg, hard=False)
            mem_avg = torch.sum(module_prob.view(self.num_module,batch_size,1) * torch.stack([r[2] for r in res]), dim=0)
...
class HCNMN(nn.Module):
    ...
    def forward(self, questions, questions_len, vision_feat, relation_mask, HCG):
        """
        Args:
            questions [Tensor] (batch_size, seq_len)
            questions_len [Tensor] (batch_size)
        ...
        for t in range(self.T_ctrl):
            ...
            # run all modules
            res = [f(vision_feat.permute(0,2,1), feat_inputs, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem, HCG) for f in self.module_funcs]
            att_stack_avg = torch.sum(module_prob.view(self.num_module,batch_size,1,1,1) * torch.stack([r[0] for r in res]), dim=0)
            stack_ptr_avg = torch.sum(module_prob.view(self.num_module,batch_size,1) * torch.stack([r[1] for r in res]), dim=0)
            stack_ptr_avg = modules._sharpen_ptr(stack_ptr_avg, hard=False)
            mem_avg = torch.sum(module_prob.view(self.num_module,batch_size,1) * torch.stack([r[2] for r in res]), dim=0)
