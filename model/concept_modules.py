...
class NoOpModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in, aff_v, concepts):
...
class FindModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in, aff_v, concepts):
        ...
        at_vision_feat = torch.bmm(att_out, vision_feat)
        sim = self.sim(at_vision_feat, query)
        aff_v, concepts = self.AUpdate(aff_v, concepts, sim)
        concepts = self.RUpdate(concepts, sim)
        concepts = self.PUpdate(concepts, sim)
        # stack_ptr = _move_ptr_fw(stack_ptr)
        # att_stack = _write_to_stack(att_stack, stack_ptr, att_out)
        return aff_v, concepts
...
class FilterModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in, aff_v, concepts):
        aff_v, concepts = self.Find(vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in)
        aff_v, concepts = self.And(vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in)
        return aff_v, concepts
...
class TransformModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in, aff_v, concepts):
        ...
        sim = self.sim(at_vision_feat, query)
        concepts = self.RUpdate(concepts, sim)
        aff_v = torch.sum(aff_v*[concept.RM for concept in concepts if concept.type=='O' ],-1)
        concepts = self.PUpdate(concepts, sim)
        norm = torch.max(aff_v, dim=1, keepdim=True)[0].detach()
        norm[norm<=1] = 1
        aff_v /= norm
        # ---------
        #att_stack = _write_to_stack(att_stack, stack_ptr, att_out)
...
class DescribeModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in, aff_v, concepts):
        ...
        out = torch.bmm(aff_v, [concept.v_features for concept in concepts if concept.type=='O' ])
        out = out.view(batch_size, -1) #(batch_size, glimpse*dim_vision)
        return aff_v, concepts, out
...
class simVL(nn.Module):
    ''' attended similarity between concepts and vision_feat, query'''
    def __init__(self):
        super().__init__()
        self.sim = torch.nn.CosineSimilarity(dim=1, eps=1e-6)

    def forward(self, concepts, at_vision_feat, query):
        return [self.sim(torch.cat(concept.v_features, concept.l_features),  torch.cat(at_vision_feat, query)) for concept in concepts]

class AUpdate(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        #[1152, 428, 249] O P R

    def forward(self, concepts, aff_v, sim):
        aff_v[:,:1152] = aff_v[:,:1152]*(sim[:,:1152]+aff_v[:,1152:1580]*[concept.PV for concept in concepts[:,:1152]]*aff_v[:,1152:1580]+[concept.RM for concept in concepts[:,:1152]*aff_v[:,:1152]*aff_v[:,1580:]])
        aff_v[:,1152:1580] =  aff_v[:,1152:1580]*(sim[:,1152:1580]+[concept.PV for concept in concepts[:,1152:1580]]*aff_v[:,:1152])
        aff_v[:,1580:] = aff_v[:,1580:]*(sim[:,1580:]+aff_v[:,:1152]*[concept.PV for concept in concepts[:,1580:]]*aff_v[:,:1152])
        norm = torch.max(aff_v, dim=1, keepdim=True)[0].detach()
        norm[norm<=1] = 1
        aff_v /= norm
        return concepts, aff_v

class PUpdate(nn.Module):
    def __init__(self) -> None:
        super().__init__()

    def forward(self, concepts, aff_v, sim):
        for i in range(concepts[:,1152:1580].size(1)):
            concepts[i].PV = concepts[i].PV*sim[:,i+1152]
            norm = torch.max(concepts[i].PVconcepts[i].PVconcepts[i].PV, dim=1, keepdim=True)[0].detach()
            norm[norm<=1] = 1
            concepts[i].PV /= norm
        return concepts, aff_v

class RUpdate(nn.Module):
    def __init__(self) -> None:
        super().__init__()

    def forward(self, concepts, aff_v, sim):
        for i in range(concepts[:,1580:].size(1)):
            concepts[i].RM = concepts[i].RM*sim[:,i+1580]
            norm = torch.max(concepts[i].PVconcepts[i].PVconcepts[i].PV, dim=1, keepdim=True)[0].detach()
            norm[norm<=1] = 1
            concepts[i].RM /= norm
        return concepts, aff_v
