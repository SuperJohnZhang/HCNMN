...
from model.net import XNMNet, NKMNet, HCNMN
...
def train(args):
    ...
    model = NKMNet(**model_kwargs).to(device)
    if args.model == 'XNM':
        model = XNMNet(**model_kwargs).to(device)
    elif args.model == 'NKM':
        model = NKMNet(**model_kwargs).to(device)
    elif args.model == 'HCNMN':
        model = HCNMN(**model_kwargs).to(device)
    logging.info(model)
    logging.info('load glove vectors')
    train_loader.glove_matrix = torch.FloatTensor(train_loader.glove_matrix).to(device)
    model.token_embedding.from_pretrained(train_loader.glove_matrix)
    ################################################################
    ...
    for epoch in range(start_epoch, args.num_epoch):
        ...
        for i, batch in enumerate(train_loader):
            ...
            # if GQA, answers needed be divided by 10
            loss = (nll * answers).sum(dim=1).mean()
            #################################################
            scheduler.step()
            optimizer.zero_grad()
            loss.backward()
...
def main():
    ...
    parser.add_argument('--kg', default='kg.h5')
    parser.add_argument('--restore', action='store_true')
    parser.add_argument('--model', type=str, default='NKM', help='specify the model name')
    ...
    parser.add_argument('--concept_h5', default=1829, type=int, help='num of concepts.')
    parser.add_argument('--num_concepts', default=1829, type=int, help='num of concepts.')
    args = parser.parse_args()
    # new knowledge inputs
    parser.add_argument('--concepts', type=str, required=True, help='path to concept feature dir')
    parser.add_argument('--hierarchy', type=str, required=True, help='path to hierarchy.json')
    parser.add_argument('--topology', type=str, required=True, help='path to topology.json')
    parser.add_argument('--concept_property', type=str, required=True, help='path to concept_property.json')
    parser.add_argument('--relation', type=str, required=True, help='path to relation.json')
    parser.add_argument('--property', type=str, required=True, help='path to property.json')
    ...
    args.kg = os.path.join(args.input_dir, args.kg)
    # set random seed
