...
def main(args):
    ...
    if args.data == 'VQA':
        ...
        if args.input_questions_json is not None:
            for f in args.input_questions_json.split(':'):
                tmp = json.load(open(f, 'r'))
                for question_id in tmp:
                    questions.append({"question_id": int(question_id),
                                  "image_id": int(tmp[question_id]['imageId']),
                                  "question": tmp[question_id]['question']})
                    annotations.append({"question_id": int(question_id),
                                    "image_id": int(tmp[question_id]['imageId']),
                                    #"question_type": str,
                                    #"answer_type": tmp[question_id]['types'],
                                    "answers": [tmp[question_id]['answer']],
                    })
        question_id_to_str = { q['question_id']:q['question'] for q in questions }
        if args.mode != 'test':
            assert len(annotations) > 0
    ...
    if args.mode == 'train':
        ...
        vocab = {
            ...
            'program_token_to_idx': {token:i for i,token in enumerate(['<eos>','find','relate','describe','is','and','exist'])}
        }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', default='VQA', choices=['VQA', 'GQA'])
