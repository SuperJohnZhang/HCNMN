...
def create_RM(term, type, cn, wi, size):
    if type==0:
        RM = cn['RM'][term]+wi['RM'][term]
    if type==1:
        return RM
    if type==2:
        RM = cn['trip'][term]+wi['trip'][term]
    return RM

def create_PV(term, type, cn, wi, size):
    if type==0:
        RM = cn['O2P'][term]+wi['O2P'][term]
    if type==1:
        return RM
    if type==2:
        RM = cn['P2O'][term]+wi['P2O'][term]
    return RM
